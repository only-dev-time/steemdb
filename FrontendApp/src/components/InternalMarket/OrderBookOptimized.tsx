...
import { useOrderBookOptimized } from '@/hooks/useOrderBookOptimized';
import { useMarketPriceOptimized } from '@/hooks/useMarketPriceOptimized';

const OrderBookOptimized = memo(() => {
  const { orderBookData, isLoading, error } = useOrderBookOptimized();
  const { priceData } = useMarketPriceOptimized();

  // Memoize processed order data
  const { sellOrders, buyOrders, lastPrice } = useMemo(() => {
    if (!orderBookData) {
      return { sellOrders: [], buyOrders: [], lastPrice: 0 };
    }
    ...
    return { sellOrders, buyOrders, lastPrice };
              ...
              <TableBody>
                {sellOrders.reverse().map((order, index) => (
                  <TableRow 
                    ...
                    className="text-xs hover:bg-red-500/5"
                  >
              ...
              <TableBody>
                {buyOrders.map((order, index) => (
                  <TableRow 
                    ...
                    className="text-xs hover:bg-green-500/5"
                  >
...
OrderBookOptimized.displayName = 'OrderBookOptimized';

export default OrderBookOptimized;
